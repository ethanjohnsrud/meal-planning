...
public static boolean print = MealPlanning.print;
...
public ObservableList<Meal> FoodOnly = CatalogView.CatalogFoodOnly;
    ...
    Meal(boolean orig, int num, String name, int pop, int last, boolean meat, boolean carb, boolean veg, boolean fruit, boolean brand, boolean pack, boolean base, boolean cooking)
    {   
        Original = orig;
        ID = num;
        Name = name;
...
//                    alert.notify("adding ingredients");
                    Ingredients.add(new Meal(false,(num*100)+1,"ingredientOne",0,0,true,true,false,false,false,false,false, false));
                    Ingredients.add(new Meal(false,(num*100)+2,"ingredientTwo",0,0,true,true,false,false,false,false,false, false));
                    Ingredients.add(new Meal(false,(num*100)+3,"ingredientThree",0,0,true,true,false,false,false,false,false, false));
                    Ingredients.add(new Meal(false,(num*100)+4,"ingredientFour",0,0,true,true,false,false,false,false,false,false));
                    Ingredients.add(new Meal(false,(num*100)+5,"ingredientFive",0,0,true,true,false,false,false,false,false, false));
                }
//                else
//                    alert.notify("NOT adding ingredients");
    ...
    //label, sets all to false and zero
    ...
    {   
        ...
        Packable = false;
        CarbBase = false;
        CookingIngredient = false;
        resetServings();

        print(); 
    }
    
    //All menus Import, details for shoppinglist sorting --- if change, must keep for old menu's
    Meal(int num, String name, boolean meat, boolean carb, boolean veg, boolean fruit)
    {   
        Original = false;
        ID = num;
        Name = name;
...
//Complete Copy Meal
    ...
    {   
        ...
        CookingIngredient = aMeal.isCookingIngredient();
    ...
    {   
        ...
        for(int i=0; i<aMeal.Ingredients.size(); i++){   //custom
            if(aMeal.Ingredients.get(i).getID()==0)
                Ingredients.add(new Meal(0,aMeal.Ingredients.get(i).getName()));
            else
                this.addIngredient(aMeal.Ingredients.get(i).getID());
        }
        //if(Original)
    ...
    {   
        ...
        Popularity = (int)(((aMeal.getPopularity()*3) + anotherMeal.getPopularity() + otherMeal.getPopularity())/5);       //3 part main meal, 2 part main ingredients , 3:5
        LastEntry = (int)(((aMeal.getLastEntry()*3) + anotherMeal.getLastEntry() + otherMeal.getLastEntry())/5);        //weighted Average
        Meat = aMeal.isMeat();
        Carb = aMeal.isCarb();
        Vegetable = aMeal.isVegetable();
        Fruit = aMeal.isFruit();
    ...
    {   
        ...
        Popularity = (int)(((aMeal.getLastEntry()*3) + (anotherMeal.getLastEntry()*2))/5);
        LastEntry = (int)(((aMeal.getLastEntry()*3) + (anotherMeal.getLastEntry()*2))/5);
        Meat = aMeal.isMeat();
        Carb = aMeal.isCarb();
        Vegetable = aMeal.isVegetable();
        Fruit = aMeal.isFruit();
    ...
    private double sortingValue; //value for comparison sorting
    ...
    private boolean CookingIngredient;

    public boolean isCookingIngredient() {
        return CookingIngredient;
    }

    public void setCookingIngredient(boolean cookingIngredient) {
