...
//        emailFiles();
    ...
    try {
        ...
        while(inputStream.hasNext()) { 
           lastRead = inputStream.next();
            ...
            Meal newMeal = new Meal(true,id,name,popularity, meat,carb,vegetable,fruit,brand,pack,base);
            ...
            while(inputStream.hasNext() && (!((lastRead=inputStream.next()).equals("\n")))) {
                ...
                newMeal.temporaryIngredients.add(new Meal(false,ingredientID,ingredientName, 0, false, false, false, false,false,false,false));
                lastRead=inputStream.next(); //reading in "|"
            }
            Catalog.add(newMeal); 
            line++;            
    ...
    while(inputStream.hasNext()) { 
        try{
           ...
           lastRead = inputStream.next();
            ...
            if(id < 0) {
                newMeal = new Meal(false,((currentMealIndex+1)*-1),"*****"+mealNames[currentMealIndex]+"*****",0,false,false,false,false,false,false,false);
                currentMealIndex++;               
            }
            else
                newMeal = new Meal(false,id,name,popularity, meat,carb,vegetable,fruit,brand,pack,base);
    ...
    public boolean menuExport(boolean prompt, String theMenu) {
            try {
                ...
                for(int i = 0; i<Menu.size(); i++) {
                     ...
                     print.print(Integer.toString(Menu.get(i).getPopularity())+","); 
    ...
    while(inputStream.hasNext()) { //not eof
        try{
            ...
            newMeal = new Meal(false,id,name,0, false,false,false,false,false,false,false);
