...
public  void menuInitiateRemoveMeal() {
            if(breakfast.isSelected()) {
                ...
                menuRemoveMeal(-1); }
            if(mondayLunch.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statusmondayLunch));
                menuRemoveMeal(-2); }
            if(mondayDinner.isSelected())  {
                Ibreakfast.setText(R.resetIndicators(statusmondayDinner));
                menuRemoveMeal(-3); }
            if(tuesdayLunch.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statustuesdayLunch));
                menuRemoveMeal(-4); }
            if(tuesdayDinner.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statustuesdayDinner));
                menuRemoveMeal(-5); }
            if(wednessdayLunch.isSelected())  {
                Ibreakfast.setText(R.resetIndicators(statuswednessdayLunch));
                menuRemoveMeal(-6); }
            if(wednessdayDinner.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statuswednessdayDinner));
                menuRemoveMeal(-7); }
            if(thursdayLunch.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statusthursdayLunch));
                menuRemoveMeal(-8); }
            if(thursdayDinner.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statusthursdayDinner));
                menuRemoveMeal(-9); }
            if(fridayLunch.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statusfridayLunch));
                menuRemoveMeal(-10); }
            if(fridayDinner.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statusfridayDinner));
                menuRemoveMeal(-11); }
            if(saturdayLunch.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statussaturdayLunch));
                menuRemoveMeal(-12); }
            if(saturdayDinner.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statussaturdayDinner));
                menuRemoveMeal(-13); }
            if(sundayLunch.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statussundayLunch));
                menuRemoveMeal(-14); }
            if(sundayDinner.isSelected()) {
                Ibreakfast.setText(R.resetIndicators(statussundayDinner));
                menuRemoveMeal(-15); }
    return;
}
...
public void updateAllIndicators() {
    try {
        ...
        for(int i=0; i<Menu.size(); i++) {
            ...
            System.out.println("size is:"+indicators.size()+" & "+status.size());
