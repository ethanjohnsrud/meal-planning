...
int none = 0;
    ...
    public void addRecent(Meal theMeal){
        try {
                if (theMeal.getID() != 0) {
                    for (int j = 0; j < Catalog.size(); j++) {
                        if (Catalog.get(j).getID() == theMeal.getID()) {
                            theMeal.increasePopularity();
...
private void makeMealSet() {
    ...
    makeMealCustomAdd.setOnAction(e -> {
        makeMealAdd(new Meal(false, 00, makeMealCustomAdd.getText(), 0, false, false, false, false, false,false,false));
