...
public void checkIngredients() {
    ...
    for (int i = 0; i<Catalog.size(); i++) {
        ...
        if(Catalog.get(i).getPopularity()<=0)
            noPopularity++;
        if(Catalog.get(i).getLastEntry()<=0)
            noLastEntry++;
    ...
    if(noPopularity>5)
        alert.error("### - "+noPopularity+" - Meals in Catalog had no Popularity for Calculations - CatalogView.checkIngredients()");
    if(noLastEntry>5)
        alert.error("### - "+noLastEntry+" - Meals in Catalog had no Last Entry (date) for Calculations - CatalogView.checkIngredients()");
