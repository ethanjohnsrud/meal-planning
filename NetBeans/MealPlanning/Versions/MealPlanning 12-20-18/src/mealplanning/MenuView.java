...
//        catalogView.catalogChangeID.setText(currentID);
        ...
        menuSelect.setOnAction(e -> {
            if (new AlertBox().actionConfirmation("Have you saved the current Menu?\n" + currentMenu)) {
                save.menuImport(menuSelect.getValue().toString());
            } else {
                menuSelect.setValue(currentMenu);
            }
        });

        menuNewButton.setOnAction(e -> {
...
//Menu Table Set
    ...
    public ObservableList<String> menusList = FXCollections.observableArrayList();
    public ComboBox<String> menuSelect = new ComboBox(menusList);
    public Button menuNewButton = R.makeButton("NEW", 50, false);
...
//Catalog View Set:
    ...
    public void setMenu() {
        ...
        menuSelect.setPrefWidth(125);
        menuSelectControls.setPrefWidth(200);
        menuSelectControls.getChildren().addAll(menuSelect, menuNewButton);
