...
public ObservableList<Meal> FoodOnly = CatalogView.CatalogFoodOnly;
    ...
    Meal(boolean orig, int num, String name, boolean meat, boolean carb, boolean veg, boolean fruit, boolean brand, boolean pack, boolean base)
    {   
        Original = orig;
        ID = num;
        Name = name;
...
//                    alert.notify("adding ingredients");
                    Ingredients.add(new Meal(false,(num*100)+1,"ingredientOne",true,true,false,false,false,false,false));
                    Ingredients.add(new Meal(false,(num*100)+2,"ingredientTwo",true,true,false,false,false,false,false));
                    Ingredients.add(new Meal(false,(num*100)+3,"ingredientThree",true,true,false,false,false,false,false));
                    Ingredients.add(new Meal(false,(num*100)+4,"ingredientFour",true,true,false,false,false,false,false));
                    Ingredients.add(new Meal(false,(num*100)+5,"ingredientFive",true,true,false,false,false,false,false));
                }
//                else
    ...
    {   
        ...
        for(int i=0; i<aMeal.Ingredients.size(); i++){
            if(aMeal.Ingredients.get(i).getID()==0)
                Ingredients.add(new Meal(false,0,aMeal.Ingredients.get(i).getName(),false,false,false,false,false,false,false));
            else
                this.addIngredient(aMeal.Ingredients.get(i).getID());
        }
        //if(Original)
    ...
    for(int q =0; q<this.temporaryIngredients.size(); q++) {
        ...
        if(!found) {
            ...
            if(temporaryIngredients.get(q).getID()!=0)
                alert.error("Meal not found in Catalogue - Added Anyways as Custom - " + Integer.toString(ID) + " - " + Name + "- Inport -> Meal.addIngredient()");
